...
<meta property="og:description" content="{{ 'Author: ' . $authors . ': Pages: ' . $book->pages .': UrduBinding: ' . $book->binding . ': CoverSize: ' . $book->size . ': Volume: ' . $book->volume }}">

<meta property="og:price:amount" content="{{ $book->special_price != $book->price ? $book->special_price : $book->price }}">
...
<meta name="twitter:description" content="{{ 'Author: ' . $authors . ': Pages: ' . $book->pages .': UrduBinding: ' . $book->binding . ': CoverSize: ' . $book->size . ': Volume: ' . $book->volume }}">
<meta name="description" content="{{ 'Author: ' . $authors . ': Pages: ' . $book->pages .': UrduBinding: ' . $book->binding . ': CoverSize: ' . $book->size . ': Volume: ' . $book->volume }}">
...
<header class="breadcrumb">
    <div class="container breadcrumb__container">
        ...
        <a href="{{ route('website.home.category-detail-view', $book->categories[0]->slug) }}" class="breadcrumb__link">{{ $book->categories->count() ? $book->categories[0]->name : '' }}</a>
        <a href="javascript:;">
            <img src="{{ asset('assets/website') }}/images/chevron-right.svg" alt="">
        </a>
    ...
    <div class="container productdetail__container">
        ...
        <div class="qucik__view-wrapper">
            <div class="quick__view-left">
                ...
                <div class="small-img">
                    <img src="{{ asset('assets/website') }}/images/next-icon.svg" class="icon-left" alt="" id="prev-img">
                    <div class="small-container">
                    <div id="small-img-roll">
                        @foreach ($book->images->where('type',1) as $image)
                            <img src="{{ asset('storage/' . $image->filename) }}" class="show-small-img" alt="">
                        @endforeach
                    </div>
                    </div>
                <img src="{{ asset('assets/website') }}/images/next-icon.svg" class="icon-right" alt="" id="next-img">
                </div>
            </div>
            <div class="quick__view-right">
                ...
                <div class="quick__cart-wrapper">
                    ...
                    <div class="quick__qty">
                        ...
                        @if ($book->in_stock == 0)
                        <button class="btn book__addtocart-btn quick__btn" id="addToCartBtnQuickView" data-id="{{ $book->id }}">Add To Cart</button>
                ...
                <div class="tab d-none" id="tab-review">
                    <div class="product__review-wrapper">
                        ...
                        <div class="product__addreview">
                            ...
                            <form action="" class="add__review-form" id="add__review-form" onsubmit="return false">
                                @csrf
                ...
                <article class="book__card">
                    <div class="book__img-wrapper">
                        <a href="{{ route('website.home.book-detail-view', $item->slug) }}">
                            <img src="{{ asset('storage/' . $item->images[0]->filename) }}" alt="{{ $item->name }}" class="book__img">
