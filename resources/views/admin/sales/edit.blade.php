        ...
        <div class="row">
            <div class="col-12">
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('admin.purchase-order.update', $order->id) }}"
                            ...
                            <div class="row mb-3">
                                <div class="col-6">
                                    <label class="form-label" for="party_id">Supplier <span
                                            class="text-danger">*</span></label>
                                    <select name="party_id" class="form-control">
                                        <option value="">---select supplier---</option>
                                        @foreach ($suppliers as $item)
                                            <option value="{{ $item->id }}" @selected($order->orderable_id == $item->id)>
                                                {{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-6">
                                    ...
                                    <input type="text" class="form-control" name="reference_no"
                                        placeholder="Reference no." value="{{ $order->reference_no }}" />
                                </div>
                            </div>
                            <div class="row mb-3">
                                <div class="col-6">
                                    <label class="form-label" for="issue_date">Issue Date <span
                                            class="text-danger">*</span></label>
                                ...
                                <div class="col-6">
                                    <label class="form-label" for="delivery_date">Delivery Date <span
                                            class="text-danger">*</span></label>
                                    <input type="date" class="form-control" name="delivery_date" placeholder="phone"
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-12">
                                    <button type="button" class="btn btn-primary" id="OpenLoadProductsModel"><i
                                            class="fas fa-spinner"></i> Load Products For Order</button>
                                    <button type="button" class="btn btn-primary my-2" id="addNonSystemProduct">Add
                                        Product</button>
                                </div>
                            </div>
                            <div class="row mb-3">
                                            ...
                                            @if ($order->order_items->count())
                                                @foreach ($order->order_items as $key => $item)
                                                    <tr>
                                                        ...
                                                        <td><input type="number" style="width:80%;height:35px"
                                                        ...
                                                        <td><input type="number"
                                                                style="width:80%;height:35px;margin-right:10px"
                                                                name="total_amount[]" id="total_amount"
                                                                value="{{ $item->total_amount }}" class="disabled"
                                                                readonly />
                                                            <i class="fas fa-trash text-danger cursor-pointer"
                                                                id="deleteOrderRow" data-id="${id}"></i>
                                                        </td>
                                                    </tr>
                                                @endforeach
                                            @endif
                            ...
                            <button type="submit" class="btn btn-primary float-end"
                                {{ $order->status == 1 ? 'disabled' : '' }}>{{ $order->status == 1 ? "Can't updated completed orders" : 'Submit' }}</button>
                        </form>
                    </div>
                </div>
            </div>
